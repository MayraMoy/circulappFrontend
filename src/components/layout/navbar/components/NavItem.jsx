...
function NavItem({ item, isActive, onClick }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex items-center gap-1.5 lg:gap-2 rounded-full px-2.5 lg:px-4 py-1.5 lg:py-2 text-xs lg:text-sm font-semibold transition duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 active:scale-95 ${
        isActive
          ? "bg-primary-dark text-white shadow-soft"
          : "text-text-secondary hover:bg-primary/10 hover:text-primary"
      }`}
    >
      <span className="flex h-4 w-4 lg:h-5 lg:w-5 items-center justify-center text-sm lg:text-base">
        <Icon />
      </span>
