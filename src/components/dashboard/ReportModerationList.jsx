...
export default function ReportModerationList({
  ...
  const [filter, setFilter] = useState('todas'); // 'todas' | 'pendiente' | 'resueltas'

  const filtered = reports.filter((r) => {
    if (filter === 'pendiente') return r.status === 'pendiente';
    if (filter === 'resueltas') return r.status === 'desestimada' || r.status === 'publicacion_eliminada';
    return true;
  });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
        ...
        >
          Resueltas ({reports.filter((r) => r.status !== 'pendiente').length})
        </button>
      </div>
                ...
                <div className="flex items-center justify-between flex-wrap gap-2 text-[11px] text-gray-500 pt-2 border-t border-gray-100">
                  ...
                  {!isPending && report.resolvedBy && (
                    <div className="text-[10px] text-gray-400">
                      Atendido por: {report.resolvedBy.name} ({report.status})
                    </div>
                  )}
                </div>
              </div>
